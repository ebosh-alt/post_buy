...
class Sqlite3_Database:
    ...
    def get_by_other_field(self, value: int | str, field: str, return_field: list) -> list:
        conn = self.sqlite_connect()
        curs = conn.cursor()
        if value.__class__.__name__ == "int":
            curs.execute(f'''SELECT {",".join(return_field)} from {self.table_name} where {field} = {value}''')
        else:
            curs.execute(f'''SELECT {",".join(return_field)} from {self.table_name} where {field} = '{value}' ''')
