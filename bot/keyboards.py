...
def get_channel_category(category) -> types.InlineKeyboardMarkup:
    ...
    all_name.update({"Вернуться в начало": "buy_advertisement"})
...
check_pay_kb = create_keyboard({"Готово": "check_pay", "Вернуться в начало": "buy_advertisement"})
...
to_start_kb = create_keyboard({"Вернуться в начало": "buy_advertisement"})
